...
class MainActivity : AppCompatActivity() {
    ...
    fun startScanService() {
        if (isServiceStarting) return // Prevent double calls
        ...
        if (!wifiManager.isWifiEnabled) {
            ...
            isServiceStarting = false // Reset flag on early return
            return
        }
        ...
        Log.d("MainActivity", "Started WifiScanService")

        // Reset flag after 1 second
        Handler(Looper.getMainLooper()).postDelayed({ isServiceStarting = false }, 1000)
    }

    fun stopScanService() {
        if (isServiceStopping) return // Prevent double calls
        isServiceStopping = true

        Log.d("MainActivity", "🛑 Stopping WifiScanService...")
        ...
        // Reset flag after 1 second
        Handler(Looper.getMainLooper()).postDelayed({ isServiceStopping = false }, 1000)
    }
